...
class IMBALANCECIFAR10(torchvision.datasets.CIFAR10):
    ...
    def get_img_num_per_cls(self, cls_num, imb_type, imb_factor):
        ...
        img_num_per_cls = []
        if imb_type == 'exp':     # exponential imbalance
            for cls_idx in range(cls_num):
                num = img_max * (imb_factor ** (cls_idx / (cls_num - 1.0)))
                img_num_per_cls.append(int(num))
        elif imb_type == 'step':  # step-like imbalance
            for cls_idx in range(cls_num // 2):  # former half: balanced
                img_num_per_cls.append(int(img_max))
            for cls_idx in range(cls_num // 2):  # latter half: imbalanced
    ...
    def __getitem__(self, index):
        ...
        meta = dict()
        if self.config.sampler.dual_sample.enable:
            # three kinds of dual sampling: reverse, balance, long-tailed
...
class CIFAR10_LT(object):
    ''' CIFAR10 with long-tailed distribution '''
    # train_instance: long-tailed training dataset
    # train_balance:  balanced training dataset
    # eval:           balanced eval dataset
    ...
    def __init__(self, config, distributed, root='../datasets', imb_type='exp',
        ...
        # define the dataset for training and evaluation (training dataset is long-tailed)
        train_dataset = IMBALANCECIFAR10(config, root=root, imb_type=imb_type, imb_factor=imb_factor, 
        ...
        # define the data loader for evaluation
